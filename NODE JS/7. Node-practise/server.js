...
//Sukurkite dinaminį GET route, kur URL turės automobilio markę ir pagal ją prafiltruos, ir grąžins tik
//tuos žmones, kurie turi šį automobilį.

app.get("/data/car/:model", (req, res) => {
  const model = req.params.model;
  console.log(`416line: ${model}`);
  // const searchCar = data.find((userCar) => userCar.car === "Audi");
...
app.get("/data/user/:id", (req, res) => {
  ...
  const user = data.find((curUser) => curUser.id.toString() === id);

  console.log(id);
  console.log(`${user.first_name} ${user.last_name} by id:${id}`);

  res.send(user);
});

// Sukurkite GET route, kuris grąžins visus el. paštus (grąžinamas formatas:
// ["anb@abc.com", "abc@abc.com", "abc@acb.com]).

app.get("/data/email", (req, res) => {
  const getMails = data.map((userMail) => userMail.email);

  console.log(getMails);
  res.send(getMails).end();
});
...
app.get("/data/female", (req, res) => {
  const getFemales = data.filter((women) => women.gender === "Female");
  const arrayForFemales = [];

  if (getFemales) {
    console.log(data.first_name);
  }

  const girls = getFemales.forEach(
    (value) => value.first_name
    //console.log([`${value.first_name} ${value.last_name}`])
  );

  while (getFemales <= getFemales.length()) {
    arrayForFemales.push(girls);
  }

  //console.log(getFemales);
  res.send(getFemales).end();
});
